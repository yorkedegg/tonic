...
#include "core/hle/service/nwm/bridge_shim.h"
...
// [BRIDGE] M0 recon instrumentation (LOCAL DEBUG ONLY — not for upstream per AI-POLICY.md).
// Formats a byte buffer as a lowercase hex string for MC3DS<->Java bridge packet capture.
static std::string BridgeHex(const u8* data, std::size_t size) {
...
// [BRIDGE] Max bytes to hex-dump per packet. Set above the UDS MaxSize (0x5C6=1478) /
// RakNet MTU (0x5d4=1492) so NO datagram is truncated -> RakNet fragments reassemble
// cleanly and StartGame/Login/chunk bodies survive for the stage-2 unwrapper.
static constexpr std::size_t kBridgeDumpMax = 2048;
...
void NWM_UDS::RecvBeaconBroadcastData(Kernel::HLERequestContext& ctx) {
    ...
    // [BRIDGE] M1: synthesise the fake host's beacon from a tunnel BEACON frame so it shows
    // in the join list. Built with the same GenerateBeaconFrame the real host uses.
    if (BridgeTunnel::Get().Active()) {
        BridgeTunnel::Get().SendScan();
        std::vector<u8> appdata;
        if (BridgeTunnel::Get().WaitBeacon(appdata, 1000)) {
            NetworkInfo ni{};
            ni.host_mac_address = {0x02, 0x00, 0x00, 0x1b, 0x87, 0x10};
            ni.channel = DefaultNetworkChannel;
            ni.initialized = 1;
            ni.oui_value = NintendoOUI;
            ni.oui_type = static_cast<u8>(NintendoTagId::NetworkInfo);
            ni.wlan_comm_id = 0x001B8710;
            ni.id = 0;
            ni.attributes = 0;
            ni.network_id = 0x00001234;
            ni.total_nodes = 1;
            ni.max_nodes = 2;
            ni.application_data_size =
                static_cast<u8>(std::min<std::size_t>(appdata.size(), ApplicationDataSize));
            std::memcpy(ni.application_data.data(), appdata.data(), ni.application_data_size);

            std::vector<NodeInfo> nodes(ni.max_nodes); // size must equal max_nodes (asserted)
            nodes[0].network_node_id = 1;              // host occupies node 1; slot 2 left empty

            Network::WifiPacket bpkt;
            bpkt.type = Network::WifiPacket::PacketType::Beacon;
            bpkt.channel = DefaultNetworkChannel;
            bpkt.transmitter_address = ni.host_mac_address;
            bpkt.data = GenerateBeaconFrame(ni, nodes);
            HandleBeaconFrame(bpkt);
            LOG_INFO(Service_NWM, "[BRIDGE] injected fake beacon ({} B) for scan", bpkt.data.size());
        }
    }
...
ResultStatus NWM_UDS::SendToHLE(u32 dest_node_id, u8 data_channel, u32 data_size, u8 flags,
    ...
    // [BRIDGE] M0 recon: dump outbound UDS application payload (raw, pre-encapsulation).
    LOG_INFO(Service_NWM, "[BRIDGE] TX node={}->{} ch={} flags={:#x} size={} data={}",
             static_cast<u32>(connection_status.network_node_id), dest_node_id,
             static_cast<u32>(data_channel), static_cast<u32>(flags), data_size,
             BridgeHex(input_buffer.data(),
                       std::min<std::size_t>(input_buffer.size(), kBridgeDumpMax)));

    // [BRIDGE] M1: send the raw payload over the tunnel instead of the room.
    if (BridgeTunnel::Get().Active()) {
        BridgeTunnel::Get().SendData(data_channel, input_buffer.data(),
                                     static_cast<u32>(input_buffer.size()));
...
Common::Expected<int, ResultStatus> NWM_UDS::PullPacketHLE(u32 bind_node_id, u32 max_out_buff_size,
    ...
    // [BRIDGE] M1: deliver tunnel DATA for this channel instead of room packets. The tunnel
    // payload is the raw game data (no LLC/SecureData wrapping), so return it directly.
    if (BridgeTunnel::Get().Active()) {
        std::vector<u8> data;
        u8 src = 1;
        if (!BridgeTunnel::Get().PopData(channel->first, data, src)) {
            output_buffer.resize(buff_size);
            return int(0);
        }
        auto data_size = static_cast<u32>(data.size());
        if (secure_data_out) {
            SecureDataHeader hdr{};
            hdr.src_node_id = src;
            hdr.data_channel = channel->first;
            *reinterpret_cast<SecureDataHeader*>(secure_data_out) = hdr;
        }
        if (data_size > max_out_buff_size) {
            return Common::Unexpected(ResultStatus::RecvError_PacketSizeTooLarge);
        }
        output_buffer.resize(buff_size);
        std::memcpy(output_buffer.data(), data.data(), data_size);
    ...
    // [BRIDGE] M0 recon: dump inbound UDS application payload (post-decapsulation).
    LOG_INFO(Service_NWM, "[BRIDGE] RX node={}->{} ch={} size={} data={}",
             static_cast<u32>(secure_data.src_node_id),
             static_cast<u32>(connection_status.network_node_id),
             static_cast<u32>(secure_data.data_channel), data_size,
             BridgeHex(output_buffer.data(),
                       std::min<std::size_t>(static_cast<std::size_t>(data_size), kBridgeDumpMax)));
...
void NWM_UDS::ConnectToNetworkHLE(NetworkInfo net_info, u8 connection_type,
    ...
    // [BRIDGE] M1: connect over the tunnel instead of the room handshake.
    if (BridgeTunnel::Get().Active()) {
        BridgeTunnel::Get().SendConnect(passphrase.data(), static_cast<u32>(passphrase.size()));
        u8 node_id = 2;
        BridgeTunnel::Get().WaitConnectOk(node_id, 2000);
        std::scoped_lock lock(connection_status_mutex);
        connection_status.status = NetworkStatus::ConnectedAsClient;
        connection_status.status_change_reason = NetworkStatusChangeReason::ConnectionEstablished;
        connection_status.network_node_id = node_id;
        connection_status.max_nodes = net_info.max_nodes ? net_info.max_nodes : 2;
        connection_status.total_nodes = 2;
        connection_status.node_bitmask = 0x3;   // host (1) + us (2)
        connection_status.changed_nodes = 0x3;
        connection_status.nodes[0] = 1;
        connection_status.nodes[node_id - 1] = node_id;
        node_info.clear();
        node_info.resize(connection_status.max_nodes);
        NodeInfo host{};
        host.network_node_id = 1;
        node_info[0] = host;
        NodeInfo me = current_node;
        me.network_node_id = node_id;
        node_info[node_id - 1] = me;
        SignalEventAsync(connection_status_event);
        LOG_INFO(Service_NWM, "[BRIDGE] connected as client, node {}", node_id);
        return;
    }
...
void NWM_UDS::ConnectToNetwork(Kernel::HLERequestContext& ctx, u16 command_id,
    ...
    // [BRIDGE] M1: the tunnel connect already completed synchronously in ConnectToNetworkHLE,
    // so respond success immediately and leave the thread running — do NOT park it on a room
    // event (parking then signalling corrupts the thread status; server_session.cpp:99 asserts).
    if (BridgeTunnel::Get().Active()) {
        IPC::RequestBuilder rb(ctx, command_id, 1, 0);
        rb.Push(ResultSuccess);
        return;
...
void NWM_UDS::SetApplicationData(Kernel::HLERequestContext& ctx) {
    ...
    // [BRIDGE] M0 recon: dump beacon application data (the join-list blob the bridge must fake).
    LOG_INFO(Service_NWM, "[BRIDGE] BEACON appdata size={} data={}", size,
             BridgeHex(application_data.data(), std::min<std::size_t>(size, kBridgeDumpMax)));
